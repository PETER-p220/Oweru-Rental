...
/**
 * Returns true only for contracts that have actually been sent to this tenant
 * and contain meaningful content (a file or at least one field defined).
 *
 * This filters out:
 *  - Contracts in 'draft' status (landlord hasn't sent them yet)
 *  - Contracts with neither a file nor any fields (empty shell records)
 */
const isContractVisible = (contract: DigitalContract): boolean => {
  // Drafts are landlord-only; tenant should never see them
  if (contract.status === 'draft') return false;

  // Must have either a downloadable file OR at least one field defined
  const hasFile   = Boolean(contract.file_url || contract.file_name);
  const hasFields = Array.isArray(contract.fields) && contract.fields.length > 0;
...
const DigitalContractPage = () => {
  ...
  const loadContracts = async () => {
    try {
      ...
      // Normalise: the API wraps in { data: [...] } or returns the array directly
      const raw: DigitalContract[] = Array.isArray(response.data)
        ? response.data
        : Array.isArray((response.data as any)?.data)
          ? (response.data as any).data
          : [];

      // Only show contracts that are actually sent/relevant to this tenant
      setContracts(raw.filter(isContractVisible));
    } catch (err: any) {
  ...
  const viewContract = (contract: DigitalContract) => {
    ...
    // Reset signature when opening a fresh contract view
    setSignatureDataUrl('');
  ...
  const getContractActionButton = (contract: DigitalContract) => {
    ...
    if (['pending_review', 'approved', 'rejected'].includes(contract.status)) {
      return (
        <button
  ...
  const submitContract = async () => {
    ...
    const missingFields = (selectedContract.fields || []).filter(
      f => f.required && f.type !== 'signature' && !fieldValues[f.id]
    );
    ...
    <div style={{ ...pageStyle, padding: '0' }}>
      ...
      <section style={{ ...panelStyle }}>
        ...
        {loading ? (
          <div style={{
            display: 'flex', alignItems: 'center', gap: '10px',
            color: palette.gray400, padding: '40px 0',
          }}>
            <div style={{
              width: 16, height: 16,
          ...
          /* ── Empty state: no contracts sent to this tenant yet ── */
          ...
          <div style={tableWrapStyle}>
            <table style={tableStyle}>
              <thead>
                <tr>
                  {['Contract', 'Property', 'Status', 'Actions'].map(h => (
                  ...
                  >
                    ...
                    <td style={tdStyle}>
                      ...
                      {contract.property?.location && (
                        <div style={{ color: palette.gray400, fontSize: '12px', marginTop: '2px', paddingLeft: '17px' }}>
                          {contract.property.location}
                        </div>
                      )}
                    </td>
                    ...
                    <td style={tdStyle}>
                      <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                        {/* Only show Download when the contract actually has a file */}
          ...
          <div style={{
            ...
            <div style={{
              ...
              <button
                ...
                onClick={() => { setShowContractModal(false); setSelectedContract(null); setError(''); }}
              >
                <X size={16} />
              </button>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              {(selectedContract.fields || []).length === 0 ? (
                <div style={{
                  ...
                  color: palette.gray400, fontSize: '14px',
                }}>
                  <FileCheck size={32} style={{ opacity: 0.3, display: 'block', margin: '0 auto 8px' }} />
                  This contract has no fillable fields.
                  {(selectedContract.file_url || selectedContract.file_name) && (
                    <div style={{ marginTop: 8 }}>
                      Use the Download button to read the full document.
                    </div>
                  )}
                </div>
              ) : (
                (selectedContract.fields || []).map((field) => (
                  <div key={field.id}>
                    <label style={{
                      ...
                      color: palette.navy700, fontSize: '14px', fontWeight: 600,
                    }}>
                      {field.label}{' '}
        ...
        <div style={{ ...overlayStyle, zIndex: 1001 }}>
          <div style={{ ...panelStyle, maxWidth: '500px', width: '100%' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <p style={{ color: palette.gray500, fontSize: '14px' }}>

  ...
  return (
    <div style={{ ...pageStyle, padding: '0' }}>
      ...
      <section style={{ ...panelStyle }}>
        <div style={{ position: 'absolute', top: 0, left: 32, right: 32, height: '2px', background: `linear-gradient(90deg, transparent, ${'var(--accent-color)'}, transparent)` }} />
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            {[
              ...
              <div key={label} style={{
                ...
                background: accent ? 'rgba(37,99,235,0.1)' : 'rgba(15,23,42,0.5)',
                border: accent ? '1px solid rgba(37,99,235,0.25)' : '1px solid rgba(148,163,184,0.08)',
                textAlign: 'center',
              }}>
                <div style={{ fontSize: '10px', letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--text-secondary)', fontWeight: 600 }}>{label}</div>
                <div style={{ fontSize: '22px', fontWeight: 700, color: accent ? 'var(--accent-color)' : 'var(--text-secondary)', marginTop: '4px' }}>{value}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.3fr) minmax(300px, 0.9fr)', gap: '20px' }}>
        {/* Messages table */}
        <section style={{ ...panelStyle }}>
          <div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '18px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <Inbox size={15} style={{ color: 'var(--accent-color)' }} /> Inbox
          </div>
          {error && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', color: 'var(--accent-color)', background: 'rgba(248,113,113,0.08)', border: '1px solid rgba(248,113,113,0.18)', borderRadius: '12px', padding: '14px 18px', marginBottom: '16px', fontSize: '14px' }}>
                  ...
                  {messages.map((item) => (
                    <tr key={item.id}
                      ...
                      <td style={{ ...tdStyle, width: 20 }}>
                        {!item.read_at && <div style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--accent-color)' }} />}
                      </td>
                      <td style={tdStyle}>
                      ...
                      <td style={tdStyle}>
                        ...
                        <div style={{ color: 'var(--text-secondary)', fontSize: '12px', marginTop: '3px' }}>{item.body}</div>
                      </td>
                      <td style={{ ...tdStyle, color: 'var(--text-secondary)', fontSize: '12px', whiteSpace: 'nowrap' as const }}>{formatDate(item.created_at)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <section style={{ ...panelStyle, alignSelf: 'start' }}>
          <div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '6px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <MessageSquare size={15} style={{ color: 'var(--accent-color)' }} /> Compose
          </div>
          <div style={{ color: 'var(--text-secondary)', fontSize: '13px', lineHeight: 1.6, marginBottom: '20px' }}>
            {recipient

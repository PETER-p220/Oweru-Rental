...
const Home = () => {
  ...
  const loadOweruProperties = async () => {
    try {
      ...
      const oweruList = list
        .filter((p) => p.type === 'oweru_rental' || p.isOweru === true)
        .slice(0, 6);
  ...
  // Load Featured / Popular Properties
  ...
  return (
    <div style={{ fontFamily: "'Jost', 'Futura PT', sans-serif", background: '#0F172A', color: '#F8F8F9', minHeight: '100vh', overflowX: 'hidden' }}>
      ...
      {/* FEATURED / POPULAR PROPERTIES - FIXED IMAGE DISPLAY */}
              ...
              {featuredProperties.map((p) => (
                <div key={p.id} className="prop-card" style={{ borderRadius: 12 }}>
                  <Link to={`/property/${p.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                    {/* FIXED: Using backgroundImage for reliable display */}
                    <div
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 24 }}>
              {bnbProperties.map((p: any) => (
                <div key={p.id} className="bnb-card" style={{ borderRadius: 12 }} onClick={() => { setSelectedProperty(p); setShowBookingModal(true); }}>
                  <div
                    className="prop-img-container"
                    style={{ height: 220, backgroundImage: `url(${getImage(p)})` }}
                  />
                  <div style={{ padding: 20 }}>
              ...
              {oweruProperties.map((p: any) => (
                ...
                >
                  <div style={{ position: 'relative', height: 210, overflow: 'hidden' }}>
                    <div
                      className="prop-img-container"
                      style={{ height: 210, backgroundImage: `url(${getImage(p)})` }}
                    />
                    <div style={{
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <input
          ...
          onChange={(e) => setFormData((p) => ({ ...p, check_in: e.target.value }))}
        />
        <input
          required
          type="date"
          ...
          onChange={(e) => setFormData((p) => ({ ...p, check_out: e.target.value }))}
        />
      </div>
      <textarea
      ...
      <div style={{ display: 'flex', gap: 12 }}>
        <button
          type="button"
          onClick={onClose}
          style={{ flex: 1, padding: '12px', background: 'transparent', color: 'var(--slate)', border: '1px solid var(--border)', borderRadius: 6, cursor: 'pointer', fontSize: 14 }}
        >
          Cancel
        </button>
        <button
          type="submit"
          disabled={loading}
          style={{ flex: 2, padding: '12px', background: 'var(--gold)', color: 'var(--navy-900)', border: 'none', borderRadius: 6, fontWeight: 700, fontSize: 14, cursor: 'pointer' }}
        >

...
const Home = () => {
  ...
  return (
    <div style={{ fontFamily: "'Jost', 'Futura PT', sans-serif", background: '#0F172A', color: '#F8F8F9', minHeight: '100vh', overflowX: 'hidden' }}>
      ...
      {/* Oweru Special Packages - Fully Working */}
      <section style={{ background: 'linear-gradient(135deg, var(--navy-900) 0%, var(--navy-800) 100%)', borderTop: '1px solid var(--border)' }}>
        <div className="section">
          ...
          {oweruLoading ? (
            ...
            <EmptyState icon={<Building size={40} />} title="No Oweru packages yet" desc="Premium properties will appear here soon." />
          ) : (
            <div className="oweru-packages-grid">
              {oweruProperties.map((p: any) => (
                ...
                >
                  <div style={{ position: 'relative', height: 210 }}>
                    <img src={getImage(p)} alt={p.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  ...
                  <div style={{ padding: 20 }}>
                    <h3 style={{ fontSize: 17, fontWeight: 600, color: 'var(--cream)', marginBottom: 10 }}>{p.title}</h3>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 12, color: 'var(--slate)', fontSize: 13 }}>
                      <MapPin size={14} style={{ color: 'var(--gold)' }} /> {p.location || p.address}
                    </div>
                    <div style={{ fontSize: 22, fontWeight: 700, color: 'var(--gold)', marginBottom: 16 }}>
                      {formatPrice(p.price)} <span style={{ fontSize: 13, color: 'var(--slate)' }}>/month</span>
                    </div>
                    <button
                      onClick={(e) => { e.stopPropagation(); navigate(`/property/${p.id}`); }}
                      style={{ width: '100%', background: 'var(--gold)', color: 'var(--navy-900)', border: 'none', padding: '14px', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', borderRadius: 6 }}
                    >
                      View Details
                    </button>
                  </div>

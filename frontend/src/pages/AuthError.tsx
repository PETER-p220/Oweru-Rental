import { useEffect, useState } from 'react';
import { useNavigate, useSearchParams } from 'react-router-dom';

const AuthError = () => {
  const [searchParams] = useSearchParams();
  const navigate = useNavigate();
  const [countdown, setCountdown] = useState(10);

  const error = searchParams.get('error') || 'unknown';

  const errorMessages: Record<string, { title: string; message: string; action: string }> = {
    user_not_found: {
      title: 'Account Not Found',
      message: 'We couldn\'t find an account with this Google email. Please register first.',
      action: 'Go to Registration',
    },
    account_inactive: {
      title: 'Account Inactive',
      message: 'Your account has been deactivated. Please contact support for assistance.',
      action: 'Contact Support',
    },
    auth_failed: {
      title: 'Authentication Failed',
      message: 'There was an error during the authentication process. Please try again.',
      action: 'Try Again',
    },
    unknown: {
      title: 'Authentication Error',
      message: 'An unexpected error occurred. Please try again.',
      action: 'Try Again',
    },
  };

  const errorInfo = errorMessages[error] || errorMessages.unknown;

  useEffect(() => {
    const timer = setInterval(() => {
      setCountdown((prev) => {
        if (prev <= 1) {
          clearInterval(timer);
          navigate('/login');
          return 0;
        }
        return prev - 1;
      });
    }, 1000);

    return () => clearInterval(timer);
  }, [navigate]);

  const handleAction = () => {
    if (error === 'account_inactive') {
      navigate('/contact');
    } else {
      navigate('/login');
    }
  };

  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 px-4">
      <div className="max-w-md w-full">
        <div className="bg-white rounded-2xl shadow-2xl p-8 text-center">
          {/* Error Icon */}
          <div className="mx-auto w-20 h-20 bg-red-100 rounded-full flex items-center justify-center mb-6">
            <svg
              className="w-10 h-10 text-red-600"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth={2}
                d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"
              />
            </svg>
          </div>

          {/* Error Title */}
          <h1 className="text-2xl font-bold text-gray-900 mb-3">{errorInfo.title}</h1>

          {/* Error Message */}
          <p className="text-gray-600 mb-8 leading-relaxed">{errorInfo.message}</p>

          {/* Action Button */}
          <button
            onClick={handleAction}
            className="w-full bg-gradient-to-r from-yellow-500 to-yellow-600 text-white font-semibold py-3 px-6 rounded-lg hover:from-yellow-600 hover:to-yellow-700 transition-all duration-200 transform hover:scale-105 shadow-lg"
          >
            {errorInfo.action}
          </button>

          {/* Countdown */}
          <p className="mt-6 text-sm text-gray-500">
            Redirecting to login in <span className="font-semibold text-yellow-600">{countdown}</span> seconds...
          </p>

          {/* Manual Redirect */}
          <button
            onClick={() => navigate('/login')}
            className="mt-4 text-sm text-gray-500 hover:text-yellow-600 transition-colors"
          >
            Go to login now
          </button>
        </div>

        {/* Additional Help */}
        <div className="mt-6 text-center">
          <p className="text-gray-400 text-sm">
            Need help?{' '}
            <a href="/contact" className="text-yellow-500 hover:text-yellow-400 transition-colors">
              Contact Support
            </a>
          </p>
        </div>
      </div>
    </div>
  );
};

export default AuthError;

...
class _LandlordAnalyticsPageState extends State<LandlordAnalyticsPage> {
  ...
  Future<void> _loadAnalytics() async {
    ...
    try {
      final response = await LandlordApiService.getAnalytics();
      setState(() {
        _analytics = response['data'] as Map<String, dynamic>? ?? response;
        _isLoading = false;
      });
    } catch (e) {
      setState(() {
  ...
  String _metricValues(int index, Map<String, dynamic> propertyPerformance, Map<String, dynamic> financialMetrics, Map<String, dynamic> tenantMetrics) {
    switch (index) {
      ...
      case 4:
        final rate = propertyPerformance['occupancy_rate'];
        final rateNum = rate is num ? rate.toDouble() : (double.tryParse(rate?.toString() ?? '') ?? 0);
        return '${rateNum.toStringAsFixed(1)}%';

...
class _LandlordRentCollectionPageState extends State<LandlordRentCollectionPage> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final paymentsResponse = results[0];
      final statsResponse = results[1];
      final paymentsData = paymentsResponse['data'];
      ...
      setState(() {
        _payments = paymentsData is List
            ...
            : [];
        _stats = statsData is Map<String, dynamic> ? statsData : statsResponse;
        _isLoading = false;
      });
    } catch (e) {
      setState(() {
  ...
  Widget _statsRow() {
    final items = [
      ...
      _StatItem(value: '${_formatRate(_stats['collection_rate'])}%', label: 'Collection Rate', icon: Icons.percent,                       accent: kSlate800, bg: kSlate100),
    ];
